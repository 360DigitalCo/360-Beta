...
/* ── Electron desktop: game-activity bridge ───────────────────────────
   Loaded on every page (this file is site-wide), so this works whether
   chat, search, or the AI page happens to be open in the desktop app
   when a game is detected. Writes to profiles.current_activity rather
   than relying on chat.js's live presence tracking, since presence only
   exists while chat.html specifically is the open tab/page — a game can
   start while the user is anywhere else in the app.
   window.desktop360 only exists inside the Electron build (see
   electron/preload.js); this is a silent no-op in the regular browser. */
window.desktop360?.onGameActivity?.(async (gameName) => {
  try {
    const { data: { session } } = await supabaseClient.auth.getSession();
    if (!session?.user?.id) return;
    await supabaseClient.from("profiles").update({
      current_activity: gameName || null,
      current_activity_updated_at: new Date().toISOString(),
    }).eq("id", session.user.id);
  } catch (e) {
    console.error("game-activity update failed:", e);
  }
});
